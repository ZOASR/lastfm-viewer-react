...
const ReactLastFMViewer = ({ api_key, user, updateInterval }: Props) => {
	...
	useEffect(() => {
		const get = async () => {
			try {
				const data: TrackInfo = await getLatestTrack(user, api_key);
				setTrack(data);
				setLoading(false);
			} catch (e) {
				console.warn(
					"Hey there developer, the user you provided may have his stats private"
				);
				setLoading(false);
			}
		};
		...
		<lfmContext.Provider value={{ colors: colors, track: track }}>
			...
			>
				<figure
					...
					style={{ boxShadow: `0 0 20px ${colors?.secondary}99` }}
				>
					{track?.lastfmImages ? (
						<img
